...
class ContactPose3D(Dataset):
  ...
  #   session_name, object_name, surface_data_filename = self.filenames[filename_idx]
  #
  #   # 1. 加载并处理占据网格和表面数据（原有逻辑不变）
  #   occ = np.zeros((1, self.grid_size, self.grid_size, self.grid_size), dtype=np.float32)
  ...
  #   colors, hand_idx, hand_vectors, hand_dists, joint_dists = np.hsplit(sdata, [1, 2, 5, 6])
  ...
  #
  #   # 2. 硬编码保留食指和大拇指关节（假设21关节模型）
  #   joints = self.joint_locations[session_name][object_name]
  #   N_joints = joints.shape[1]  # 每只手的关节数（通常为21）
  #
  #   # 定义保留的关节索引（食指：5-8，大拇指：1-4，手腕：0）
  #   thumb_indices = [0, 1, 2, 3, 4]  # 手腕 + 大拇指（关节0-4）
  #   index_indices = [5, 6, 7, 8]  # 食指（关节5-8）
  #   kept_joint_indices = thumb_indices + index_indices
  #
  #   # 生成布尔掩码（True表示保留）
  #   kept_joints = [
  #     np.array([i in kept_joint_indices for i in range(N_joints)]),  # 第一只手
  #     np.array([i in kept_joint_indices for i in range(N_joints)])  # 第二只手
  #   ]
  #
  #   # 3. 应用掩码到关节距离数据
  #   for i in range(2):  # 双手
  ...
  #   if self.eval_mode:
  #     kept_joints = [d.squeeze().astype(np.int_) for d in kept_joints]
...
def eval_collate_fn(data):
  ...
  batch = []
  for idx, d in enumerate(data):
    occs.append(torch.as_tensor(np.array(d[0]), dtype=torch.float32))
    sdatas.append(torch.as_tensor(np.array(d[1]), dtype=torch.float32))
    ijks.append(torch.as_tensor(np.array(d[2]), dtype=torch.float32))
    colors.append(torch.as_tensor(np.array(d[3]), dtype=torch.long))
    kept_joints.append(torch.as_tensor(np.array(d[4]), dtype=torch.long))
    batch.append(idx * torch.ones(np.array(len(d[1])), dtype=torch.long))
  occs = torch.stack(occs)
  sdatas = torch.cat(sdatas)
  ijks = torch.cat(ijks)
  colors = torch.cat(colors)
...
if __name__ == '__main__':
  ...
  dset = ContactPose3D('data/', './data/contactpose_data', 64, 1,
                       joint_droprate, eval_mode=False,
                       include_sessions=[50],
