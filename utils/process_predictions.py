...
def calc_cc(preds, targs):
    cc, _ = pearsonr(preds, targs)
    return cc


def calc_sim(preds, targs):
    sim = np.sum(preds * targs) / (np.linalg.norm(preds) * np.linalg.norm(targs))
    return sim


def calc_kld(preds, targs):
    kld = entropy(preds, targs)
    return kld
...
def calc_auc(preds, targs, texture_bins, rebal_w, max_dist=1.0, N=500,
    ...
    order = np.argsort(dists)  # 获取按照差异升序排列的一个索引
    preds = preds[order]
    targs = targs[order]
    dists = dists[order]

    threshes = np.linspace(0, max_dist, N + 1)  # 均匀间隔阈值的生成
    positives = np.zeros(len(threshes))  # 储存AUC正样本数量
    rebal_positives = np.zeros(len(threshes))  # 储存rebalance-AUC正样本数量
    # 分别表示当前差异和阈值的索引
    d_idx = 0
    t_idx = 1
    while True:
        tr_idx = max(0, np.searchsorted(texture_bins, targs[d_idx]) - 1)
        pr_idx = max(0, np.searchsorted(texture_bins, preds[d_idx]) - 1)  # 计算真实值和预测值所属纹理索引
        # rw = max(rebal_w[tr_idx], rebal_w[pr_idx])
        rw = rebal_w[tr_idx]  # 计算当前差异对应的纹理组的权重
        if dists[d_idx] <= threshes[t_idx]:  # 差异值
            positives[t_idx] += 1
            rebal_positives[t_idx] += rw  # 更新正样本数量
        else:
            t_idx += 1  # 超过阈值更新阈值索引
            if t_idx == len(threshes):  # 处理完所有阈值跳出循环
            ...
            d_idx -= 1
        d_idx += 1  # 更新差异值索引准备处理下一个样本
        if d_idx == len(dists):
            break
    if t_idx < len(threshes) - 1:
        positives[t_idx + 1:] = positives[t_idx]
        rebal_positives[t_idx + 1:] = rebal_positives[t_idx]  # 确保数组长度一致22
    ...
    rebal_auc = np.sum(rebal_positives) / N  # 计算AUC REBAL-AUC
...
def process(filename, annealed_mean_T=0.38, rebal_l=1, n_runs=3, show=False,
    ...
    for run_idx, filename in enumerate(filenames):
        ...
        for this_id in sorted(preds.keys()):
            ...
            if this_id not in rmses:
                rmses[this_id] = []
            if this_id not in ccs:
                ccs[this_id] = []
            if this_id not in sims:
                sims[this_id] = []
            if this_id not in klds:
                klds[this_id] = []
            if this_id not in ses:
                ses[this_id] = []
            ...
            kld = calc_kld(p, t)
            # 计算显著性误差
            significance_error = np.mean(np.abs(p - t))  # 取预测值和目标值之间的绝对差的平均值作为显著性误差
            aucs[this_id].append(auc)
            rebal_aucs[this_id].append(rebal_auc)
            rmses[this_id].append(rmse)
            ccs[this_id].append(cc)
            sims[this_id].append(sim)
            klds[this_id].append(kld)
            ses[this_id].append(significance_error)
    ...
    for this_id in sorted(aucs.keys()):
        ...
        cc.append(np.mean(ccs[this_id]))
        cc_stdev.append(np.std(ccs[this_id]))
        sim.append(np.mean(sims[this_id]))
        sim_stdev.append(np.std(sims[this_id]))
        kld.append(np.mean(klds[this_id]))
        kld_stdev.append(np.std(klds[this_id]))
        se.append(np.mean(ses[this_id]))
        se_stdev.append(np.std(ses[this_id]))
    ...
    print(s)
    s = 'RMSE: ' + ' '.join(['{:.4f}'] * len(ids)).format(*rmse)
    print(s)
    s = 'CC: ' + ' '.join(['{:.4f}'] * len(ids)).format(*cc)
    print(s)
    s = 'CC +/-: ' + ' '.join(['{:.4f}'] * len(ids)).format(*cc_stdev)
    print(s)
    s = 'SIM: ' + ' '.join(['{:.4f}'] * len(ids)).format(*sim)
    print(s)
    s = 'SIM +/-: ' + ' '.join(['{:.4f}'] * len(ids)).format(*sim_stdev)
    print(s)
    s = 'KLD: ' + ' '.join(['{:.4f}'] * len(ids)).format(*kld)
    print(s)
    s = 'KLD +/-: ' + ' '.join(['{:.4f}'] * len(ids)).format(*kld_stdev)
    print(s)
    s = 'SE: ' + ' '.join(['{:.4f}'] * len(ids)).format(*se)
    print(s)
    s = 'SE +/-: ' + ' '.join(['{:.4f}'] * len(ids)).format(*se_stdev)
    print(s)
