...
#     kept_joints=[None, None], mesh_scale=1e-3, save_dir='data/save'):
#   if not isinstance(mesh, o3dg.TriangleMesh):
#     mesh = o3dio.read_triangle_mesh(mesh)
#
#
#
#     mesh.scale(mesh_scale, center=np.array([0.,0.,0.]))
...
#   combined_mesh=o3dg.TriangleMesh()
#   for geom in geoms:
#     if isinstance(geom,o3dg.TriangleMesh):
#       combined_mesh+=geom
#   output_directory=save_dir
#   file_count=len(os.listdir(output_directory))
#   output_filename=os.path.join(output_directory,f'{file_count+1}_output_mesh.ply')
#   o3dio.write_triangle_mesh(output_filename,combined_mesh)
...
def show_prediction(surface_colors, mesh, binvox, joint_locs,
  ...
  geoms = [apply_colormap(mesh, apply_sigmoid=False)]
##原始图像信息
  original_mesh = o3dg.TriangleMesh()
  for geom in geoms:
      if isinstance(geom, o3dg.TriangleMesh):
            original_mesh += geom
##添加手部关节后的图像信息
  ...
  for hand_idx, hand in joint_locs.items():
    geoms.extend(create_hand_geoms(hand, joint_color=hand_colors[hand_idx],
      kept_joints=kept_joints[hand_idx]))
  o3dv.draw_geometries(geoms)
   # 合并几何对象到一个TriangleMesh
  combined_mesh = o3dg.TriangleMesh()
  for geom in geoms:
        if isinstance(geom, o3dg.TriangleMesh):
            combined_mesh += geom
  output_directory=save_dir#包含手部关节的预测图像
  # 生成唯一的文件名
  file_count = len(os.listdir(output_directory))
#保存路径，可以更改，默认为新建文件夹：data/save文件夹下保存
  output_filename1 = os.path.join(output_directory, f'{file_count + 1}_output_mesh.ply')
  output_filename2 = os.path.join(output_directory, f'{file_count + 1}_output_mesh_no_joints.ply')
  #保存合并后的TriangleMesh为文件
  o3dio.write_triangle_mesh(output_filename1, combined_mesh)
  o3dio.write_triangle_mesh(output_filename2, original_mesh)
