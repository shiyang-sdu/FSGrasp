...
def eval(data_dir, contactpose_dir, split, checkpoint_filename, config_filename,
  ...
  for batch_idx, batch in enumerate(dloader):
    ...
    if show_object is True:
      ...
      # # 大拇指和是指
      # kept_joints[0] = [False] * 21
      # kept_joints[0][0] = True  # 手腕
      # kept_joints[0][1:5] = [True] * 4  # 拇指
      # kept_joints[0][5:9] = [True] * 4
      #
      # kept_joints[1] = [False] * 21
      # kept_joints[1][0] = True  # 手腕
      # kept_joints[1][1:5] = [True] * 4  # 拇指
      # kept_joints[1][5:9] = [True] * 4

      # # 大拇指和是指 前两个
      # kept_joints[0] = [False] * 21
      # kept_joints[0][3:5] = [True] * 2  # 拇指
      # kept_joints[0][7:9] = [True] * 2
      #
      # kept_joints[1] = [False] * 21
      # kept_joints[1][3:5] = [True] * 2  # 拇指
      # kept_joints[1][7:9] = [True] * 2

      # import pdb
      # pdb.set_trace()
      show_prediction(avg_pred, kept_joints, dset.texture_bins, object_name, session_name, save_dir = 'data/save')
